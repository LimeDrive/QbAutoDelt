...
def scoreTorrent(cfg, qbt):
    ...
    for t in qbt.torrents_info():
        ...
        t_score = s_ratio + s_seed + s_tag + s_state + s_cat
        t_info = (t.name, t.hash, t.size)
        data[t_info] = t_score
        logger.debug(f"\n \
            {t.name} :\n \
            Ratio: {str(t.ratio)}/={str(s_ratio)}   SeedTime: {str(t.time_active)}/={str(s_seed)}   Tag: {t.tags}/={str(s_tag)}   Category:  Tag: {t.category}/={str(s_cat)}   State: {t.state}/={str(s_state)}\n \
